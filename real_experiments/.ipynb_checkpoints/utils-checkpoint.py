...
def compute_p_values(
    ...
    log_prior_odds: float = 0.0, # assumes equal pre-change/post-chance representation
    n_jobs: int = -1,
    backend: str = "loky",
    verbose: int = 0,
):
    """Compute CONCH p-values for all changepoint candidates, based on classfier based LLR score given the model logits """
    ...
    # get classifier based LLR score from model logits
    ...
    # compute CONCH p-value for single index
    def pvalue_single_t(t, seed): 
        rng = np.random.default_rng(seed)
        s_obs = score_from_logits_at_t(logits, t)
        idx_left, idx_right = np.arange(t), np.arange(t, n)
        count = 0
        for _ in range(nperm):
            pi = np.concatenate([rng.permutation(idx_left), rng.permutation(idx_right)])
            s_perm = score_from_logits_at_t(logits[pi], t)
            if s_perm <= s_obs:
                count += 1
        return t, (count + 1) / (nperm + 1)

    # parallelization for efficient implementation
...
def confidence_set(p_values, alpha: float):
    """Return {t : p(t) > alpha} as confidence set."""
